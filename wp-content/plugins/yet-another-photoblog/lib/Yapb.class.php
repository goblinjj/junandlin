	...
	*/
	...
	class Yapb {
		...
		function Yapb() {
			...
			require_once realpath(dirname(__FILE__) . '/Savant3-3.0.1/Savant3.php');
			...
			if (!is_null($image = YapbImage::getInstanceFromDb($post->ID))) {
				...
				if (get_option('yapb_display_images_xml_media_enclosure')) {
					if (get_option('yapb_display_images_xml_media_enclosure_resized_activate')) {
						...
						// Generate thumbnail if not existent

						if (!file_exists($image->_getUniqueThumbnailPath($options))) {
							$url_for_direct_request = str_replace('&amp;', '&', $image->getThumbnailHref($options)); // we don't need xhtml &amp; in direct requests
							$result = file_get_contents($url_for_direct_request);
						}
