	...
	*/
	...
	class YapbMaintainance {
		...
		function fetchImagefileList() {
			...
			$imageRows = $wpdb->get_results('SELECT id,post_id,uri FROM ' . YAPB_TABLE_NAME . ' ORDER BY id');
			for ($i=0, $len=count($imageRows); $i<$len; $i++) {
				$row = $imageRows[$i];
				$yapbImage = new YapbImage($row->id, $row->post_id, $row->uri);
				array_push($this->imagefileList, $yapbImage->systemFilePath());
			}
		}
		...
		function getImagefileSizeBytes() {
			...
			for ($i=0, $len=$this->getImagefileCount(); $i<$len; $i++) {
				if (file_exists($this->imagefileList[$i])) {
		...
		/**
		 * Method returns a list of all files not found on hdd
		 *
		 * @return array
		 */
		function getMissingFiles() {
			...
			foreach ($imageRows as $imageRow) {
				...
				$yapbImage = new YapbImage($imageRow->id, $imageRow->post_id, $imageRow->uri);
				$filepath = $yapbImage->systemFilePath();
